Feature: Parking space suggestion
  ...
  Scenario: The user can access parking space's suggestion page 
   ...
   Scenario: The system try create two accounts with the same username 
  	Given the system has one account "Mateus" with preference for parking spaces in the "Area II" sector
  	When I signed up as "Mateus" with preference for parking spaces in the "Area II" sector
